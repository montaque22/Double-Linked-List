...
describe('Double Linked List', function(){
    ...
    describe('#Verify Psychic Cycle- Setter', function(){
        it('Should Return true when trying to access manually but returns undefined when accessed via getter', function(){

            var list = new DoubleLinkedList();
            var mid = {};

            list.insertAtEnd({id:1, name: 'Bet'});
            list.insertAtEnd({id:2, name: 'Kit'});
            list.insertAtEnd({id:3, name: 'Kat'});
            list.insertAtEnd({id:4, name: 'Cat'});
            list.insertAtEnd({id:5, name: 'Sit'});


            list.psychic(function(c, p, n, idx){
                if(c.id === 3 && p.name === 'Kit' && n.id === 4){
                    mid = c;
                    return false;
                }
                return true;
            ...
            assert.deepEqual({id:3, name: 'Kat'}, mid);
        })
    })
})
